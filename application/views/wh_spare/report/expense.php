...
<div id="report" style="margin-top: 20px;">
	...
	<h4 align="right" style="margin-right: 10%;">
		...
		<h3 align="center" style="font-style: italic;"> ХАНГАЛТЫН
			ЗАРЛАГЫН ТАЙЛАН</h3>
...
<table border="0" cellpadding="3" cellspacing="0" align="center"
    ...
    <?php
        ...
        <th align='center'>Үнэ</th>               
        <th align='center'>Нийт үнэ</th>               
        <th>Хэсэг</th>
        <th>Зориулалт</th>
        <th>Хүлээн авсан</th>
        </thead>";
				...
				foreach ( $r_result as $row ) {							
					if ($tmpMod == 0) {
						$tmpMod = 1;
						$curColspan = $exp_array [$row->invoice_id];
					...
					if ($curColspan > 1) {
						if ($prevRow != $curRow) {
							echo "<tr><td rowspan='$curColspan'>$row->expense_no</td><td rowspan='$curColspan'>$row->expense_date</td><td>$row->spare</td><td>$row->short_code</td><td>$row->qty</td><td>$row->amt</td><td>$row->total</td>	<td rowspan='$curColspan'>$row->section</td><td rowspan='$curColspan'>$row->intend</td><td rowspan='$curColspan'>$row->receivedby</td></tr>";
							$i ++;
						} else {
							if ($curColspan == $prevColspan) {
								...
								// colspan print
								echo "<tr><td rowspan='$curColspan'>$row->expense_no</td><td rowspan='$curColspan'>$row->expense_date</td><td>$row->spare</td><td>$row->short_code</td><td>$row->qty</td><td>$row->amt</td><td>$row->total</td><td rowspan='$curColspan'>$row->section</td><td rowspan='$curColspan'>$row->intend</td><td rowspan='$curColspan'>$row->receivedby</td>
								</tr>";
								$i ++;
							}
						}
					} else {
						echo "<tr><td>$row->expense_no</td><td>$row->expense_date</td><td>$row->spare</td><td>$row->short_code</td><td>$row->qty</td><td>$row->amt</td><td>$row->total</td><td>$row->section</td><td>$row->intend</td><td>$row->receivedby</td></tr>";
						$i ++;
					}
