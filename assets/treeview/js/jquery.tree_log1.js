...
;(function($) {
	...
	$.extend($.fn, {
		...
		treeview: function(settings) {
			...
			//console.log('equipmetn'+$('#vw_loc_equip_id').val());
			...
			$(document).on("click", 'li > span.module', function(event) {
                ...
                //Хэрэв top элэемент сонгохыг хориглоно энэ тохиолдолд үйлдлүүд гүйцэтгэгдэх болно.
		        ...
		        //тухайн байршилаар session_gнээх хэрэгтэй байна!			

		        //ehleed formuudig duudah yostoi!!
		        var my_url = $(location).attr('href');
			    			    
			    var node_id = _this.attr('id');
 	            var node = _this.text(); 
 	            // get form name by any other form 

 				if(my_url.indexOf("create_form") >= 0&&$('#create_form').length){
			           	  form='#create_form';
			    }else if(my_url.indexOf("edit")>= 0&&$('#edit_form').length ){
			           	  form='#edit_form';
			    }else form ='#close_form';
	            ...
	            if (form=='#create_form'||form=='#edit_from'){
	           	// none top bish bol
	                if(_this.hasClass('failure')){
	            	   // хэрэв Failure бол дараах зүйлсийг хийнэ	                                 	                	
	                	if(!_this.hasClass('basic')&&!_this.hasClass('undevelop')){ //module parent bol
					if(unchk_pr_error(node_id, equipment_id)==true){
			                   // 1. unselect failure
								...
								alert('Энэ үйлдлийг хийхийг зөвшөөрөхгүй учир нь [basic] буюу эцсийн элемент дээр алдаа өгсөн учраас!');
							}
	                	}else if(!_this.hasClass('undevelop')){ //basic bolon none_basic		
			            ...
			            if($(this).hasClass('undevelop')){
			               ...
			               if(json.status=='success'){	
				              ...
				              var res_json = get_logic(equipment_id, _this);					            
	               ...
	               // its used for flag its first time or not							
		            ...
		            // хэрэв Failure бол дараах зүйлсийг хийнэ
	                   var node_id = _this.attr('id');	                   
						...
						if(!_this.hasClass('basic')&&!_this.hasClass('undevelop')){
							// basic bolon parent-г алдаатай сонгогдоход шалгах session-d bgaa esegiig shalgah
							if(unchk_pr_error(node_id, equipment_id)==true){
								// tuhain function reverse hiihed							
								rev_logic(_this, equipment_id, node_id)
								// 1. unselect failure
		            	...
		            	}else if(_this.hasClass('undevelop')){
		            	//4. Undeveloped байгаад хэрэв unselect хийхэд өөр .node сонгогдсн байвал юу ч хийх хэрэггүй                	
	                	// Хэрэв зөвхөн энэ node сонгогдсон байгаа бол type-г өөрчилнө.
	                	// ямар нэгэн node байгаа эсэхийг шалгана!
	                		_this.removeClass('failure');  
	                		...
	                		// tag-г авч хаяна	                		
			               	...
			               	if($('.node').length==0)
			               	  $('#log_type_id option[value="' + 0 + '"]').prop('selected',true);      	
			               	  // else юу ч хийх шаардалгагүй	            		
			            }else{ // basic failure selected
			               //console.log('its basic calss');	
			               rev_logic(_this, equipment_id, node_id)
								// 1. unselect failure
								_this.removeClass('failure');                	 
			                	// 2. remove tag
			...
			//function uncheck parent error is true or no
			function unchk_pr_error(id, equipment_id){
			   var ret ;
				$.ajax({				   
				   type:    'POST',
	   			   url:    base_url+'/index.php/flog/jx_unchk_pr_error/',
				   data:   {equipment_id:equipment_id, id:id, location_id:location_id},
	               dataType: 'json',
	               async: false,
			...
			function chk_parent(_id, equipment_id, type){
				...
				$.ajax({				   
				   type:    'POST',
	   			   url:    base_url+'/index.php/flog/jx_pr_error/',
				   data:   {equipment_id:equipment_id, id:_id, node_type:type, location_id:location_id },
	               dataType: 'json',
	               async: false,
	              success: function(json){
	              	if (json.status == "success") { 
			...
			function remove_tag(node_id){
				...
				$( ".node" ).each(function(){
		            ...
		            $.ajax({
		                type: 'POST',
		                url: base_url+'/index.php/flog/index/node_select',
		                async: false,
		                data: data,
			...
			function rev_logic(_this, equipment_id, node_id){
				$.ajax({				   
				   type:    'POST',
       			   url:    base_url+'/index.php/flog/jx_reverse/',
				   data:   {equipment_id:equipment_id, id:node_id, location_id:location_id },
                   dataType: 'json',
                   async: false,
                  success: function(json){
                  	console.log(json);
                  	if (json.status == "success") { // амжилттай нэмсэн тохиолдолд		                  		
                  		if(json.result.result=='true'){
							//find_next_parent_remove!!!!                  		   
                  		   find_parent_remove(_this, json);		                  			 
                  		}else{
 			...
 			function set_tag_node(_this){
	            ...
	            $.ajax({
	                ...
	                success: function(data) {
	                    ...
	                    //!!!console.log('parent'+data.parent);
	                        //$('#myTags').tagit('createTag', data.node);
	                        //check this id is existed!!
	                        if($('#tag_'+node_id).length){
	                        	alert('Энэ мөчрийг аль хэдийн нэмсэн байна!');
	                        }else{
	                           $('#wrapper-tag').append("<a class='btn-tag' id='tag_"+node_id+"'>"+data.node+"</a>");	                        
	                           // $('#node_id').val($('.current').attr("id"));                	                        
	                           $(form).append("<input type='hidden' name='node[]' class='node' id ='node_"+node_id+"' value='"+node_id+"'/>");	                        	
	                        }
            ...
            function get_logic(equip_id, _this){
            	...
            	if(!_this.hasClass('undevelop')){            		
	            	$.ajax({				   
					   type:    'POST',
	       			   url:    base_url+'/index.php/flog/jx_logic/',
					   data:   {equipment_id:equip_id, id:current_id, location_id:location_id }, //location_id-g nemsen!
	                   dataType: 'json',
	                   async: false,
	                  success: function(json){
	                  	if (json.status == "success") { // амжилттай нэмсэн тохиолдолд
            ...
            function find_next_parent(_this, json){	            
                ...
                var arr = $.map(json.error, function(el) { return el });
	            if($.inArray(_this.children('span').attr('id'), arr)>-1){	                   
	               //2. тэр нь алдаан дотор байвал parent-г class-д failure өгнө		
	               _this.children('span').addClass('failure');		               	               
	               // console.log('here is loading'+document.getElementById('tag_'+_this.children('span').attr('id')));	               
	               if(!document.getElementById('tag_'+_this.children('span').attr('id'))){
	               //    //3. Tаг-д тухайн олдсон элэментийг node-г авч нэмнэ.
	               //    //$('#myTags').tagit('createTag', _this.children('span').text());
	                   $('#wrapper-tag').append("<a class='btn-tag' id='tag_"+_this.children('span').attr('id')+"'>"+_this.children('span').text()+"</a>");
   
	                   $(form).append("<input type='hidden' name='node[]' class='node' id ='node_"+_this.children('span').attr('id')+"' value='"+_this.children('span').attr('id')+"'/>");
                    }
            ...
            //remove parent 
            function find_parent_remove(_this, json) {
	            _this = _this.closest('li').closest('ul').closest('li');	                 	                
	           if (_this.length > 0) {	            		             
	               // console.log('id:'+_this.children('span').attr('id'));
	               // console.log('json2: '+parseInt($.inArray(_this.children('span').attr('id'), json.result.error)));
	               
	               var arr = $.map(json.result.error, function(el) { return el });
	              if(parseInt($.inArray(_this.children('span').attr('id'), arr))==-1){	 	            	
	                 //_this.children('span').attr('style', 'background-color:red'); 
	                 // ! Хэрэв failure class аль хэдийн өгсөн бол дахин өгөх шаардлагаггүй	                
	                 //2 if(_this.children('span').hasClass('failure')){
	                	//herev bval 
	                _this.children('span').removeClass('failure');                   
	                // console.log('child'+_this.children('span').text());
	                // $("#myTags").tagit("removeTagByLabel", _this.children('span').text());
	                if(('#tag_'+_this.children('span').attr('id')).length){
				       $("#tag_"+_this.children('span').attr('id')).remove(); 		
	                   $('#node_'+_this.children('span').attr('id')).remove();
		            }          	
	                
	              }
	                return find_parent_remove(_this, json);
	        	}
	        }

           // ! this recursive until parent cus its true!!
	        function find_parent(_this) {
	           //console.log('hi there'+_this.closest('li').closest('ul').closest('li').children('span').text());
	           if (_this.closest('li').closest('ul').closest('li').children('span').text()) {
	              _this = _this.closest('li').closest('ul').closest('li');	               
	               // ! Хэрэв failure class аль хэдийн өгсөн бол дахин өгөх шаардлагаггүй
	               _this.children('span').addClass('failure');
...
//               $("#myTags").tagit("removeAll");
               ...
               if($('#create_form').length)
	             ...
	             // herev zasah form bval
	          	...
	          	$.ajax({				   
				   type:    'POST',
			   	   url:    base_url+'/index.php/flog/jx_reset/',
				   data:   {equipment_id:e_id, location_id:location_id},
			       dataType: 'json',
			       async: false,
			       success: function(json){
			          if (json.status == "success") { // амжилттай нэмсэн тохиолдолд
