...
//     var width = $('#main_wrap').width();   
    ...
    jQuery("#grid").jqGrid({        
        url:base_url+'/training/trainer2/index/grid',
        datatype: "json",
        mtype: 'GET',
        height: '500',  
        width:'1260',
        colNames:['#','Үнэмлэх #','Байршил', 'Нэр.Овог', 'Төрөл', 'Мэргэжил', 'Олгосон /t','Хүчинтэй /t', 'Ажиллах төхөөрөмж', 'Утас', 'Имэйл', 'Үйлдэл'],
        colModel:[
...
function t_action (cellvalue, options, rowObject) {     
     action_str ="<div title='Дэлгэрэнгүй' style='float:left;cursor:pointer;' class='ui-pg-div ui-inline-edit'><span id='log_none' onclick='init_view("+rowObject.trainer_id+")' class='ui-icon ui-icon-extlink'></span></div></div>";
     ...
     $('input.action').each(function() {
       switch($(this).val()){     
          ...
          case 'license_print':
            action_str=action_str+"<div title='Үнэмлэх хэвлэх' style='float:left;cursor:pointer;' class='ui-pg-div ui-icon-print'><span onclick='_print("+rowObject.trainer_id+")' target ="+options.log_num+" class='ui-icon ui-icon-print'></span></div></div>";    
          break;
      }
     });
     return action_str;
